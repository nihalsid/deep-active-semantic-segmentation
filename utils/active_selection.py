...
class ActiveSelectionMCDropout:
	...
	@staticmethod
	def _square_nms(score_maps, region_size, max_selection_count):
		
		score_maps[score_maps < 0.10] = 0
		...
		while score_maps.nonzero().size(0) != 0 and selection_count < max_selection_count:
			...
			i, r, c = argmax // (score_maps.shape[1] * score_maps.shape[2]), (argmax // score_maps.shape[2]) % score_maps.shape[1], argmax % score_maps.shape[2]
			
			selected_indices[i.item()].append((r.item(), c.item()))
			selection_count += 1
	...
	def create_region_maps(self, model, images, region_size):
		...
		for image_batch in tqdm(loader):
			...
			for img_idx, entropy_map in enumerate(self._get_vote_entropy_for_batch(model, image_batch)):
				...
				score_maps[map_ctr, :, :] = torch.nn.functional.conv2d(entropy_map.unsqueeze(0).unsqueeze(0), weights.unsqueeze(0).unsqueeze(0)).squeeze().squeeze()
				map_ctr += 1
	...
	def get_vote_entropy_for_images(self, model, images):
		...
		for image_batch in tqdm(loader):
			image_batch = image_batch.cuda()
			entropies.extend([torch.sum(x).cpu().item() / (image_batch.shape[2] * image_batch.shape[3]) for x in self._get_vote_entropy_for_batch(model, image_batch)])
...
def test_entropy_map_for_images():
	...
	checkpoint = torch.load(os.path.join(constants.RUNS, 'active_cityscapes', 'al_4-variance-scratch_ep100-bs_125-deeplab-mobilenet-bs_12-513x513', 'run_0425', 'best.pth.tar'))
...
def test_nms():
	...
	region_size = 127
	weights = torch.cuda.FloatTensor(region_size, region_size).fill_(1.)
	score_maps = torch.stack([torch.nn.functional.conv2d(torch.from_numpy(img_0).cuda().unsqueeze(0).unsqueeze(0), weights.unsqueeze(0).unsqueeze(0)).squeeze().squeeze(), torch.nn.functional.conv2d(torch.from_numpy(img_1).cuda().unsqueeze(0).unsqueeze(0), weights.unsqueeze(0).unsqueeze(0)).squeeze().squeeze()])
	min_val = score_maps.min()
	max_val = score_maps.max()
	minmax_norm = lambda x : (x - min_val) / (max_val - min_val)   
...
def test_nms_on_entropy_maps():
	...
	active_selector = ActiveSelectionMCDropout(train_set.NUM_CLASSES, train_set.env, args.crop_size, args.batch_size)
	print(active_selector.create_region_maps(model, train_set.current_image_paths[:12], 127))


if __name__ == '__main__':
	#test_entropy_map_for_images()
	test_nms_on_entropy_maps()