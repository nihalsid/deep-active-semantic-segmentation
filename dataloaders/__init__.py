...
def make_dataloader(dataset, base_size, crop_size, batch_size, overfit, **kwargs):
    ...
    if dataset == 'active_cityscapes':
        ...
        train_set = active_cityscapes.ActiveCityscapes(path=dataset_path, base_size=base_size, crop_size=crop_size,
                                                       split='train', init_set=kwargs['init_set'], overfit=overfit)
        val_set = active_cityscapes.ActiveCityscapes(path=dataset_path, base_size=base_size, crop_size=crop_size,
                                                     split='val', init_set=kwargs['init_set'], overfit=overfit)
        test_set = active_cityscapes.ActiveCityscapes(path=dataset_path, base_size=base_size, crop_size=crop_size,
                                                      split='test', init_set=kwargs['init_set'], overfit=overfit)
        num_classes = train_set.NUM_CLASSES
