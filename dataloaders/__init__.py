...
def make_dataloader(dataset, base_size, crop_size, batch_size, overfit, **kwargs):
    ...
    if dataset == 'active_cityscapes_image':
        ...
        test_set = active_cityscapes.ActiveCityscapesImage(path=dataset_path, base_size=base_size, crop_size=crop_size,
    ...
    if dataset == 'active_cityscapes_region':
        ...
        val_set = region_cityscapes.ActiveCityscapesRegion(path=dataset_path, base_size=base_size, crop_size=crop_size,
                                                           split='val', init_set=kwargs['init_set'], overfit=overfit)
        test_set = region_cityscapes.ActiveCityscapesRegion(path=dataset_path, base_size=base_size, crop_size=crop_size,
                                                            split='test', init_set=kwargs['init_set'], overfit=overfit)
        num_classes = train_set.NUM_CLASSES
